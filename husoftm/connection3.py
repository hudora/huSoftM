...
This module enables connections to a server which in turn enables
ODBC connections to the AS/400. It is meant to connect to
softmexpress.
...
class MoftSconnection(object):
    ...
    def __init__(self, tag='', host="localhost:8002"):
    ...
    def query(self, tables=None, condition=None, fields=None, querymappings=None,
              grouping=None, ordering=None, limit=None):
        ...
        if not fields:
            fields = []
        tablenames = [self._get_tablename(x) for x in tables]
        ...
        if limit:
            args['limit'] = limit
        args_encoded = urllib.urlencode({'q': json.dumps(args)})
        url = "/sql?" + args_encoded
        digest = hmac.new('geheim', url, hashlib.sha1).hexdigest()
        conn.request("GET", url,'', {'X-sig': digest})
